...
import mekanism.api.fabric.transfer.fluids.IFluidTank;
...
import mekanism.common.lib.AnnotationData;
    ...
    protected void collectScanData(Map<String, Class<?>> classNameCache, Map<Class<?>, List<AnnotationData<?>>> knownClasses) {
        ...
        for (Entry<Class<?>, List<AnnotationData<?>>> entry : knownClasses.entrySet()) {
            ...
            for (AnnotationData<?> data : entry.getValue()) {
                ...
                if (!(data.annotationData() instanceof ContainerSync containerSync))
                    continue;

                String getterName = Objects.requireNonNullElse(containerSync.getter(), "");
                ...
                try {
                    if (handler == null) {
                        ...
                        String setterName = Objects.requireNonNullElse(containerSync.setter(), "");
                        if (type != null) {
                ...
                for (String tag : Objects.requireNonNullElse(containerSync.tags(), fallbackTagsList.toArray(new String[0]))) {
